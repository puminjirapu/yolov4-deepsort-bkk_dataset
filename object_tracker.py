...
flags.DEFINE_float('iou', 0.15, 'iou threshold')
...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            if not track.is_confirmed() or track.time_since_update > 30:
                continue 
            bbox = track.to_tlbr()
    ...
    # save to pickle
    save_path = os.path.join('/'.join(FLAGS.output.split('/')[:-1]), 'state.bin')
